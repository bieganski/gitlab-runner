...
```shell
wget https://storage.googleapis.com/golang/go1.16.darwin-amd64.tar.gz
sudo tar -C /usr/local -xzf go*-*.tar.gz
```
...
```shell
wget https://storage.googleapis.com/golang/go1.16.darwin-amd64.pkg
open go*-*.pkg
```
...
```shell
pkg install go-1.16.0 gmake git mercurial
```
